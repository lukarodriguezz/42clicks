...
function SpotlightCard({ service }: { service: any }) {
  ...
  return (
    <div
      ...
      className={`group relative overflow-hidden bg-surface-container-low border border-white/5 p-6 sm:p-8 transition-colors hover:border-white/10 ${service.colSpan} rounded-sm`}
    >
      <motion.div
      ...
      <div className="relative z-10 flex flex-col h-full justify-between">
        ...
        <div className="flex items-center gap-2 text-[9px] font-bold uppercase tracking-[0.2em] text-white/30 group-hover:text-primary-container transition-colors">
          <span>Consulte para más información</span>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-5">
          {services.map((service, idx) => (
            <SpotlightCard key={idx} service={service} />
          ))}
        </div>
      </div>
    </section>
